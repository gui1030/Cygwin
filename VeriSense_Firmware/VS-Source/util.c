...
//------------------------------------------------------------------------------
//  Emulates Linux time() library function
//------------------------------------------------------------------------------
time_t time (
        time_t *tp ) {
    time_t  now = clock_seconds();
    if (!tp)
        *tp = now;
    return now;
}
...
//------------------------------------------------------------------------------
//  Return string of form @Seconds.Hundredths
//------------------------------------------------------------------------------
char* now_string () {

    unsigned long   now = clock_time();     // uSeconds since boot
    static char     buf[16];

    sprintf(buf, "@%ld.%.2ld", now/CLOCK_SECOND,
            ((now*100)/CLOCK_SECOND)%100);
    return buf;
}

//------------------------------------------------------------------------------
//  Routine to compute a 16-bit checksum from an array of arbitrary length.
//  Algorithm by John G. Fletcher (1934-2012) of Lawrence Livermore Labs
//------------------------------------------------------------------------------
uint16_t cksum16 (
        const void *payload,
        size_t     len) {
...
//------------------------------------------------------------------------------
//  Following function derived as stated below
//------------------------------------------------------------------------------
/*-
